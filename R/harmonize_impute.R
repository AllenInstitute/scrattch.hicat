...
impute_knn_old <- function(knn.idx, reference, dat, knn.dist=NULL, w=NULL, mc.cores=1,transpose=FALSE,th=NULL,sparse=FALSE,batch.size=100000,combine=TRUE,...)
  {
    require(doParallel)
    query = row.names(knn.idx)
    if(is.null(w)){
    ...
    impute.dat = foreach(j=1:length(bins),.combine="c") %dopar% {
      ...
      for(i in 1:ncol(knn.idx)){
        ...
        if(length(w)>1){
          ...
          impute.dat[select,] <<-  impute.dat[select,] +  tmp.dat* w
        }
        rm(tmp.dat)
        gc()        
      }
