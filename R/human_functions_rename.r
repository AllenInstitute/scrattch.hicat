...
makeLayerLabel <- function(Samp.dat,column="roi_label"){
  val = as.character(as.matrix(Samp.dat[,column])) # Note: layer for LGN in this case will be NA (this is GOOD)
  val = gsub("2_3","3",val)
...
getTopMarkersByPropNew <- function(
  ...
  ord      = order(-medianDif[keepProp]*ifelse(sortByMedian,1,0), 
...
renameAndOrderClusters <- function(
  ...
  if(!is.null(classNameColumn)){
    if(!is.na(classLevels[1])) clusterInfo[,classNameColumn] = 
  ...
  ordVal   = paste0("ordNew = order(clusterInfo[,classNameColumn],",
