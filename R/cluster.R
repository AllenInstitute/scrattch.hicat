...
jaccard_louvain.new <- function(dat, k=10)
{
  require(Rphenograph)
  rpheno <- Rphenograph(dat, k = k)
  return(cl = membership(rpheno[[2]]), result=rpheno)
}
...
#'
#' @examples
merge_cl<- function(norm.dat, cl, rd.dat, min.cells=4,de.param = de_param(), type=c("undirectional","directional"), max.cl.size=300,de.method="limma")
  {
