...
#'
#' Adapted from https://www.cell.com/cell-systems/fulltext/S2405-4712(19)30073-0 (https://github.com/chris-mcginnis-ucsf/DoubletFinder) 
#'
...
#' each cell's neighborhood in PC space. Default is the minimum of 1% of cells sampled or 100, whichever is smaller.
#'
#' @return An list of doublet.scores per samples and plots depicting the doublet scores for cells and artificial doublets.
doubletFinder <- function(data, 
                          select.genes, 
                          proportion.artificial = 0.20,
                          k = NULL, 
                          plot = FALSE) {
  if(is.null(k)) {
  ...
  if(ncol(data) > 10000){
    ...
    rd.dat <- rd_PCA(norm.dat, 
                     ...
                     select.cells = colnames(data_wdoublets), 
                     th = 0, 
                     max.pca=50, 
                     sampled.cells = sampled.cells)$rd.dat
  } else {
    rd.dat <- rd_PCA(norm.dat, 
                     select.genes = select.genes, 
                     select.cells = colnames(data_wdoublets), 
  ...
  knn.result <- RANN::nn2(rd.dat, 
                          k = k)
  knn.idx <- knn.result[[1]]
  knn.dist <- knn.result[[2]]
  
  knn.result1 <- RANN::nn2(rd.dat[1:n_real.cells,], 
                          rd.dat[(n_real.cells + 1):nrow(rd.dat),], 
                          k = 10)
  ...
  doublet.freq <- doublet.freq[1:n_real.cells,]
  row.names(doublet.freq) <- colnames(data)
  doublet.score <- pmax(rowMeans(doublet.freq),
                        rowMeans(doublet.freq[,1:ceiling(k / 2)]))
  
  names(doublet.score) <- colnames(data)
  ...
  if (plot == TRUE) {
    ...
    p = ggplot2::ggplot(ds, 
                        ggplot2::aes(x = doublet.score, 
                                     fill = group, 
                                     color = group)) +
      ggplot2::geom_density(alpha = 0.4) + 
      ggplot2::scale_color_manual(values = c("#F9627D",
                                    "#2F3061")) +
      ggplot2::scale_fill_manual(values = c("#F9627D",
                                   "#2F3061")) +
      ggplot2::labs(title = plot.title)
    
    return(list(doublet.score = doublet.score, 
                p = p))
