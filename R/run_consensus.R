...
run_consensus_clust <- function(norm.dat, select.cells=colnames(norm.dat), niter=100, sample.frac=0.8, output_dir="subsample_result",mc.cores=1, de.param=de_param(), merge.type=c("directional","undirectional"), override=FALSE, init.result=NULL, cut.method="auto",...)
{
  ...
  run <- function(i,...){
    ...
    result <- iter_clust(norm.dat=norm.dat, select.cells=select.cells,prefix=prefix, de.param = de.param, merge.type=merge.type, result=init.result, ...)
  ...
  if(graph.size < 10^8){
    ...
    consensus.result = iter_consensus_clust(co.ratio, co.result$cl.list, norm.dat, select.cells=all.cells, de.param = de.param, merge.type=merge.type, method=cut.method, result=init.result)
    ...
    result <- iter_clust(norm.dat=norm.dat, select.cells=all.cells, de.param = de.param, merge.type= merge.type, result= init.result,...)
  ...
  cl = refine.result$cl
  merge.result= merge_cl(norm.dat=norm.dat, cl=cl, rd.dat=Matrix::t(norm.dat[markers,]), de.param = de.param, merge.type=merge.type, return.markers=FALSE)
