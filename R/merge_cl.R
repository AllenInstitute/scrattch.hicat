                    ...
                    verbose = 0)
  {
    ...
    while(length(unique(cl)) > 1){
      ...
      while(length(new.pairs) > 0){
        ...
        tmp.de.genes =de_selected_pairs(norm.dat, cl=tmp.cl, pairs=merge.pairs[new.pairs,,drop=F], de.param= de.param, method=de.method)
    ...
    if(return.markers){
      ...
      de.genes = de_all_pairs(norm.dat, cl=tmp.cl, de.genes=de.genes, de.param=de.param)
    }
