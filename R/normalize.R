...
lm_normalize <- function(dat, x, R_2.th = 0.2, padj.th = 0.01,min.genes=5)
  {
    m = setNames(rowMedians(dat), row.names(dat))
    q.max = setNames(rowMaxs(dat), row.names(dat))
    dat = dat - m
    x = x - median(x)
    ...
    if(length(select.genes)>=min.genes){
      ...
      dat[dat < 0] = 0
      dat[select.genes,] = apply(dat[select.genes,], 2, function(x){
...
lm_matrix <- function(dat, x)
{
  ...
  se_r= rowVars(dat)
  se_f = rowVars(resid)
  R_2 =setNames(1 - se_f/se_r,row.names(dat))
  R_2[is.na(R_2)] = 0
  n = ncol(dat)
  F_stats = setNames((se_r - se_f)/(se_f/(n-2)), row.names(dat))
