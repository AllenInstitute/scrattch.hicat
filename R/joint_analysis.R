...
test_knn <- function(knn, cl, reference, ref.cl)
  {
    ...
    cl.pred.prob=do.call("rbind",tapply(names(cl), cl, function(x){
...
###Select genes for joint analysis
  ...
  cl.big= cl.platform.counts[names(ref.de.param.list),,drop=F] >= cl.min.cells
...
merge_cl_multiple <- function(comb.dat, cl,ref.de.param.list, select.genes, verbose=TRUE)
{
  ...
  print(cl.small)
  split = FALSE
  ####Merge clusters based on DE genes for each ref set. 
  for(ref.set in ref.sets){
    tmp.cells=  intersect(names(cl), colnames(dat.list[[ref.set]]))
    tmp.cl = cl[tmp.cells]
    ###Choose the clusters with > min.cells, and not depleted by at least 2 fold for merging test. Also merge clusters that are too small for all ref sets.  
    select.cl = c(colnames(cl.big)[cl.big[ref.set,] & cl.platform[ref.set,]  > 1/(length(ref.sets) *2)],cl.small)
    tmp.cl = tmp.cl[tmp.cl %in% select.cl]
    select.cl = unique(tmp.cl)
    if(length(select.cl) <= 1){
      next
    }
    cat("Merge clusters ", ref.set, "\n")
    ...
    #tmp=display_cl(cl=tmp.cl[names(tmp.cl) %in% tmp.cells], norm.dat = ref.dat, de.param = de.param, prefix="tmp")
...
impute_knn_multiple <- function(all.results, reference, dat)
  {
    impute.dat = NULL
    k.times = NULL
    for(x in names(all.results)){
      ...
      tmp.dat = impute_knn(result$knn, reference, dat)
      if(is.null(impute.dat)) {
        impute.dat = tmp.dat
        k.times[row.names(impute.dat)] = 1
      }
      else{
        impute.dat[row.names(tmp.dat),] =  impute.dat[row.names(tmp.dat),] + tmp.dat
        k.times[row.names(tmp.dat)] =  k.times[row.names(tmp.dat)] + 1
      }
    }
    impute.dat = impute.dat / k.times[row.names(impute.dat)]
    return(impute.dat)
  }



process <- function(comb.dat, prefix, overwrite=TRUE, ...)
  {
    if(!overwrite){
...
plot_markers <- function(comb.dat, cl, prefix, sets = names(comb.dat$dat.list), de.param.list,  maxGenes=500, cl.col=NULL, select.genes=NULL, save.matrix=FALSE,...)
  {
    with(comb.dat,{
      ...
      tmp.cl.means = get_cl_means(tmp.dat[select.genes,], cl[intersect(names(cl), colnames(tmp.dat))])
...
get_cl_means_list <- function(comb.dat, select.genes, cl)
  {
    cl.means =  with(comb.dat, sapply(sets, function(x){
      ...
      if(!is.null(ref.de.param.list[[x]])){
        select.cl = names(cl.size)[cl.size >= ref.de.param.list[[x]]$min.cells]
      }
      else{
