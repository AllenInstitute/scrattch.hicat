...
```
Note that the clustering differ slightly from the reference clusters. Based on analysis based on a bigger dataset, we know that the cell type diversities for CGE-derived interneurons are beyond what we described here, and clustering analysis is far from saturation due to small number of cells. 
...
```{r Build dendrogram, echo=FALSE, include=FALSE}
...
```{r Plot cluster correlation, echo=FALSE, fig.keep="all"}
...
```{r tSNE plot,fig.keep="all", fig.height = 8, fig.width = 7}
...
Compare the consensus clustering results against the reference clusters, and we see that PCA mode produce clusters at lower resolution than WGCNA mode. Small clusters like Igtp, which has very distinct transcriptional signature, still got merged with other related clusters.  
