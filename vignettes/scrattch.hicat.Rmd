...
```{r Load libraries, echo = F, messages = F}
library(WGCNA)
library(dendextend)
library(matrixStats)
library(Matrix)
...
For this demo, we'll select a small subset for clustering: Igtp, Ndnf and Vip neurons. This gives us a set of 262 single-cell transcriptomic profiles to work with.
```{r Filter samples}
select.cells <- with(anno, sample_id[primary_type!="unclassified" & grepl("Igtp|Ndnf|Vip",primary_type)])
```
...
We recommend using WGCNA for smaller, high-depth datasets (< 4,000 samples; > 5,000 genes detecter per sample), and PCA for large or low-coverage datasets (> 4,000 samples or < 5,000 genes detected per sample). Another consideration is that WGCNA is considerably slower than PCA. The example we show here does not contain many cells, but for the sake of time, we'll use PCA mode for now.  
```{r Run iter_clust, message=FALSE, warning=FALSE, results="hide", echo=TRUE}
set.seed(12345)
PCA.clust.result <- iter_clust(norm.dat, 
                               ...
                               rm.eigen = rm.eigen)
```
To use WGCNA mode, which will provide better resolution on this dataset, simply set the **dim.method** parameter to "WGCNA".  
...
rd.dat <- t(norm.dat[PCA.clust.result$markers, select.cells])
...
```{r Run consensus clustering, echo=FALSE}
set.seed(12345)
...
```{r Build dendrogram, echo=FALSE, , fig.height = 5, fig.width = 7}
...
```{r Plot cluster correlation, echo=FALSE, fig.keep="all", fig.height = 7, fig.width = 7}
...
```{r tSNE plot,fig.keep="all", fig.height = 7, fig.width = 7}
...
```{r Coclustering heatmap, fig.height = 7, fig.width = 7}
