...
For small datasets (#cells < 1000), we recommend de.score.th = 60. 
...
Perform clustering using WGCNA for dimensional reduction. WGCNA mode is good for detecting rare clusters, provides cleaner cluster boundaries, while PCA is more scalable to large datasets, captures combinatorial marker expression pattern more effectively, and more sensitive to low depth dataset. We also found that WGCNA mode is better at masking technical artifacts, which is less likely to be confounded with other biological variation. We recommend using WGCNA for small high quality datasets, and PCA for large low coverage datasets. 
...
Iterative clustering pipeline tend to produce many clusters, and uncertainty increases as we try to split the clusters at finer resolution. Therefore, it will be very important to access the robustness of clustering result. We solve this problem by performing clustering many times on 80% of subsampled cells, compute cell-cell co-clustering matrix, and finally infer consensus clustering. We also compute statistics to eveluate confidence of seperation between every pair of consensus clusters, and and 


```{r, echo=FALSE}
result <- run_consensus_clust(norm.dat[,select.cells], niter=50, de.param =de.param, rm.eigen=rm.eigen)
```
...
result <- run_consensus_clust(norm.dat, niter=50, de.param =de.param, rm.eigen=rm.eigen, mc.cores=4)
```
...
Annotate the consensus clusters based on the reference cluster annotation
```{r}
compare.result=compare_annotate(merge.result$cl, ref.cl, ref.cl.df)
compare.result$g
...
dend.result <- build_dend(cl.med[select.markers,levels(cl.clean)],l.rank, l.color,nboot=100)
